...
NKS에 배포된 NTT 서버를 Ncloud 환경에서도 동작하도록 설정하여 이중화 작업을 진행했습니다. 그러나 Ncloud와 NKS 환경이 완전히 동일하지 않아 기존 Dockerfile이 예상대로 동작하지 않는 문제가 발생했습니다.

이 문제를 해결하기 위해 두 환경의 차이점을 분석한 후, 최대한 유사한 설정을 유지하면서도 각 환경에 맞춘 별도의 Dockerfile을 작성하였습니다. 이를 통해 두 환경 모두에서 안정적으로 서버를 운영할 수 있게 되었습니다.

주요 작업 내용
이중화 목적: NKS와 Ncloud 환경에 동일한 서비스를 배포하여 고가용성 확보.
문제점: 환경 차이로 인해 기존 Dockerfile이 Ncloud에서 동작하지 않음.
해결 방안:
두 환경의 차이점 분석(NKS와 Ncloud의 OS, 라이브러리, 네트워크 설정 등).
Ncloud와 NKS 환경별로 최적화된 Dockerfile을 작성.
최대한 환경을 유사하게 유지하면서도 각 환경에 맞춘 설정 적용.
결과적으로, 두 환경에서의 안정적인 서비스 동작을 보장하며 이중화 작업을 성공적으로 완료하였습니다. 이러한 경험은 다양한 클라우드 환경에서의 이식성과 호환성을 고려한 DevOps 작업의 중요성을 다시금 확인하는 계기가 되었습니다.

### NTT K8S Nat 설정 
쿠버네티스(Kubernetes) 환경에서 애플리케이션을 배포하는 경우, Pod가 항상 유동적인 IP를 가지기 때문에 ACL(Access Control List) 처리가 어려운 문제를 겪을 수 있습니다. 
이를 해결하기 위해, NAT(Network Address Translation)를 설정하여 컨테이너의 IP가 고정적으로 할당되도록 구성했습니다.

그러나, 작업 도중 실제로 Pod가 NAT를 거쳐 올바른 Destination IP에 도달하는지 확인하기 어려운 상황에 직면했습니다. 
이를 해결하기 위해 간단한 웹 서버를 개발하였고, curl 요청을 통해 Remote Address와 Host 정보를 확인하는 방식을 도입했습니다. 이를 통해 NAT 설정이 올바르게 작동하고 있음을 검증할 수 있었습니다.

주요 작업 내용
문제점: Pod의 유동적인 IP로 인해 ACL 처리 및 트래픽 제어가 어려움.
해결책: NAT를 활용하여 컨테이너에 고정 IP 할당.
검증 과정:
간단한 웹 서버를 생성.
curl 요청을 통해 Remote Address와 Host 정보를 확인.
NAT 설정이 의도한 대로 작동함을 확인.
이 과정을 통해 Kubernetes 환경에서 네트워크 안정성을 확보하였으며, ACL 및 트래픽 제어가 더욱 용이해졌습니다. 
해당 솔루션은 Kubernetes 기반 시스템의 배포 및 관리 효율성을 크게 향상시키는 데 기여했습니다.

### Kolo Jenkins 배포 문제 
현재 Jenkins와 Kolo 서버가 각각 Docker 컨테이너 환경에서 운영되고 있으며, Jenkins 컨테이너를 통해 Kolo 서버를 재가동하는 작업이 필요했습니다. 
이를 위해 Docker 컨테이너 간 연동을 구성하고, docker.sock 공유를 통해 Jenkins 컨테이너에서 Kolo 서버의 컨테이너를 제어할 수 있도록 설정하였습니다.

## 문제 상황
컨테이너 간 연동 설정은 성공적으로 완료되었으나, 재가동 과정에서 일부 컨테이너(nginx, uWSGI)가 정상적으로 **stop**되지 않는 문제가 발생했습니다. 또한, 재가동 시 기존 컨테이너 이름과 중복되는 오류로 인해 재배포가 실패하는 상황이 발생했습니다.

해결 방안
컨테이너의 정상적인 재가동을 보장하기 위해 다음과 같은 방안을 도입하였습니다:
...
기존 컨테이너가 정상적으로 종료되지 않는 문제를 해결하기 위해, 재가동 전에 실행 중인 컨테이너를 강제 종료하도록 관리 스크립트를 개선하였습니다.
docker stop 및 docker rm 명령을 사용하여 기존 컨테이너를 명확히 정리한 뒤, 새로운 컨테이너를 배포하도록 변경하였습니다.
주요 개선 내용
docker.sock 공유를 통한 초기 접근성 확보: Jenkins 컨테이너에서 Docker API 호출이 가능하도록 설정.
SSH 방식 전환: 서버 내에서 직접 컨테이너를 제어하는 안정적인 배포 프로세스 도입.
오류 방지 로직 추가: 중복된 컨테이너 이름 문제를 해결하고, 컨테이너 종료 실패에 대비한 강제 정리 프로세스 적용.
결과 및 효과
Jenkins와 Kolo 서버 간의 원활한 연동을 유지하며, 배포 과정의 신뢰성을 높였습니다.
컨테이너 관리 및 배포 절차를 자동화하면서도, 예상치 못한 오류에 대한 유연한 대응이 가능해졌습니다.
이 작업을 통해 전체 배포 프로세스의 안정성과 운영 효율성을 크게 향상시켰습니다.
